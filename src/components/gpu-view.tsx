...
FIRST VIEWPORT: a metric view inside /resources — a verdict line (device, driver, live
encode state), then a band pairing a 140px thermal arc against a full-width 60s
core-utilization area chart, then the VRAM attribution bar.
FORM: a metric panel in the resources switcher, alongside CPU/MEM/DISK/NET/DISK I/O;
a precisely specified request inside an established world, so shaped directly with no
concept seed.
...
import { useTranscodes, type TelemetrySample, type TelemetryStatus } from "@/lib/client";
...
function buildVramOverview(device: GpuDevice): OverviewModel {
  ...
  return {
    ...
    // Only shown when something is actually unattributed — an idle GPU with nothing
    // to explain should not render three lines of amber about a segment that isn't there.
    caveat:
      unattributed > 0
        ? "Unattributed covers driver, display, and graphics contexts NVML does not itemize per process."
        : undefined,
  };
}
    ...
    <div className="space-y-4">
      ...
      {samples.length === 0 ? (
        ...
        <>
          ...
          <div className="panel p-4">
            <div className="grid md:grid-cols-[auto_1fr] gap-6">
              ...
              <div className="min-w-0">
                ...
                {/* Percentage has a real ceiling — 100 — so a fixed max keeps a 2% idle
                    blip from drawing as a full-height spike. */}
                <Sparkline data={utilSeries} max={100} className="text-accent" height={72} />
          ...
          {/* d2. per-process VRAM — precision beside the bar's proportion. Quieter than
              the transcode table below: this is supporting detail, not the payoff. Renders
              nothing when there is nothing itemized — no empty-state panel, no zero row. */}
          {device.processes.length > 0 && (
            <div className="space-y-1.5">
                ...
                <table className="w-full text-xs">
                  ...
                  <tbody>
                    {device.processes.map((p) => (
                      <tr key={p.containerId ?? p.pid} className="border-b border-line/50 last:border-0">
                        <td className="px-3 py-1.5 text-ink-dim truncate max-w-64">
                          {p.containerName ?? basename(p.name)}
                        </td>
                        <td className="px-3 py-1.5 font-mono text-ink-faint">{p.pid}</td>
                        <td className="px-3 py-1.5 font-mono text-ink-dim">{formatBytes(p.memBytes)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <div className="md:hidden space-y-1">
                {device.processes.map((p) => (
                  <div
                    key={p.containerId ?? p.pid}
                    className="panel px-3 py-1.5 flex items-center justify-between gap-2 text-xs min-h-11"
                  >
                    <span className="text-ink-dim truncate min-w-0">{p.containerName ?? basename(p.name)}</span>
                    <span className="font-mono text-ink-faint shrink-0">pid {p.pid}</span>
                    <span className="font-mono text-ink-dim shrink-0">{formatBytes(p.memBytes)}</span>
                  </div>
                ))}
              </div>
            </div>
